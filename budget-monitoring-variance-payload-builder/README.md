# budget-monitoring-variance-payload-builder

> Builds deterministic Lark budget variance payloads and, when asked, runs the full budget monitoring flow through a NyxID service instead of direct Lark credentials.

---

**Mirrored from [Ornn](https://ornn.chrono-ai.fun/skills/budget-monitoring-variance-payload-builder) — read-only.**

Edits here are NOT propagated back. Submit changes on Ornn.

- Latest version: `1.4`
- Last synced: `2026-05-31T18:00:01.583Z`

## Install

```bash
npx skills add ChronoAIProject/ornn-skills/budget-monitoring-variance-payload-builder
```

## Use

See `SKILL.md` in this folder for the full instructions an AI agent
follows when this skill is loaded.
