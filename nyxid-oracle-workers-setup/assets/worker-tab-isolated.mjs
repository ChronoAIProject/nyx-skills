...
const SCRIPT_VERSION = process.env.NYXID_WORKER_SCRIPT_VERSION || "cdp-1.3-url-key-image";
// URL-key tab isolation. When TAB_KEY_URL is set the worker pins itself to one
// slot tab, always re-navigates to that key URL for a fresh prompt (so a task
// never lands in another project's stray /c/<id>), and only follows the
// server-provided conversation_url for follow-ups. Unset → original behavior.
const TAB_KEY_URL = process.env.NYXID_CHATGPT_TAB_KEY_URL || "";
...
// In-page stamp that claims a tab as ours. Applied both as an addInitScript
// (re-runs on every future navigation, including ChatGPT's /?nyx=… → /c/<id>
// redirect that drops the query flag) and immediately via evaluate.
function STAMP_TAB(m) {
  try {
    sessionStorage.setItem("nyxid_oracle_channel", m);
    window.name = "nyxid:" + m;
    document.documentElement.setAttribute("data-nyxid-oracle-channel", m);
  } catch (e) {}
}

async function markTabOwnership(page) {
  if (!TAB_STORAGE_MARKER) return;
  try { await page.evaluate(STAMP_TAB, TAB_STORAGE_MARKER); } catch (e) {}
}

async function tabIsMine(page) {
  if (TAB_URL_MATCH && page.url().includes(TAB_URL_MATCH)) return true;
  if (!TAB_STORAGE_MARKER) return false;
  try {
    return await page.evaluate(
      (m) =>
        window.name === "nyxid:" + m ||
        sessionStorage.getItem("nyxid_oracle_channel") === m ||
        document.documentElement.getAttribute("data-nyxid-oracle-channel") === m,
      TAB_STORAGE_MARKER
    );
  } catch (e) {
    return false;
...
async function getChatPage(context) {
  // URL-key isolation: bind to OUR slot tab only — never adopt a stray ChatGPT
  // tab parked on some other project's /c/<id>.
  if (TAB_KEY_URL) {
    let page = null;
    for (const p of context.pages()) {
      if (!isChatGptUrl(p.url())) continue;
      if (await tabIsMine(p)) { page = p; break; }
    }
    if (!page) {
      page = await context.newPage();
      if (TAB_STORAGE_MARKER) await page.addInitScript(STAMP_TAB, TAB_STORAGE_MARKER);
      await page.goto(TAB_KEY_URL, { waitUntil: "domcontentloaded" });
    } else if (TAB_STORAGE_MARKER) {
      await page.addInitScript(STAMP_TAB, TAB_STORAGE_MARKER);
    }
    await installDomCore(page);
    await markTabOwnership(page);
    return page;
  }
  // Fallback: original behavior (non-isolated / private worker).
...
async function selectModel(page, modelLabel) {
  try {
    ...
    // Map the requested model label to an intelligence-level menu item. The
    // current ChatGPT (GPT-5.5) UI exposes Instant/Medium/High/Extra High/Pro
    // as role="menuitemradio" entries; "-pro" selects the Pro level. We keep
    // the Chinese labels too so this works on either localization.
    const lower = rawLabel.toLowerCase();
    const compact = lower.replace(/^(chatgpt|openai)-/, "").replace(/[\s._-]+/g, "");
    let wantedTexts;
    if (/\bpro\b|pro$|扩展|extended/.test(lower) || compact.endsWith("pro")) {
      wantedTexts = ["Pro", "Pro 扩展", "扩展"];
    } else if (/extra\s*high|ultra|超高/.test(lower)) {
      wantedTexts = ["Extra High", "超高"];
    } else if (/\bhigh\b|高级|advanced/.test(lower)) {
      wantedTexts = ["High", "高级"];
    } else if (/medium|balanced|均衡/.test(lower)) {
      wantedTexts = ["Medium", "均衡"];
    } else if (/instant|fast|极速/.test(lower)) {
      wantedTexts = ["Instant", "极速"];
    } else {
      wantedTexts = [rawLabel];
    }
    log(`selecting model "${modelLabel}" -> level "${wantedTexts[0]}"`);

    // Open the model picker with a REAL pointer click. The pill is a Radix
    // menu that ignores synthetic element.click(); Playwright's locator.click()
    // dispatches genuine pointer events that open it.
    let opened = false;
    for (const sel of ['button.__composer-pill[aria-haspopup="menu"]', 'button[aria-haspopup="menu"]']) {
      ...
      for (let i = 0; i < n && !opened; i++) {
        ...
        try {
          ...
          const t = ((await btn.innerText({ timeout: 1000 })) || "").trim();
          if (!/instant|medium|high|extra|pro|gpt|思考|扩展|极速|均衡|高级|超高|\b5(\.|\b)/i.test(t)) continue;
          await btn.click({ timeout: 5000 });
          if (await waitForModelMenu(page, 5000)) { opened = true; break; }
        } catch (e) {}
      }
      if (opened) break;
    }
    ...
    // Click the matching level via a REAL click on the menuitemradio. Two
    // passes: exact normalized match first (so "High" never matches "Extra
    // High"), then a fuzzy contains fallback.
    const norm = (s) => (s || "").toLowerCase().replace(/[\s._-]+/g, "");
    const wantedNorm = wantedTexts.map(norm).filter(Boolean);
    ...
    for (const exact of [true, false]) {
      for (let i = 0; i < count; i++) {
        ...
        const hit = exact
          ? wantedNorm.some((w) => cand === w)
          : wantedNorm.some((w) => cand.includes(w) || w.includes(cand));
        if (hit) {
          await item.click({ timeout: 5000 });
          log(`model set to "${text}"`);
          return;
        }
      }
    }
...
async function handlePrompt(page, task) {
  ...
  if (TAB_KEY_URL) {
    // URL-key isolation:
    //  - follow-up → MUST go to the exact server-provided conversation_url;
    //    refuse otherwise rather than reuse whatever the tab is parked on.
    //  - fresh     → ALWAYS re-goto our slot key URL, let ChatGPT redirect to
    //    this slot's own /c/<id>, then type. Never reuse a stray /c/<id>.
    if (task.is_followup) {
      if (!task.conversation_url) {
        throw new Error("follow-up without conversation_url: refusing to reuse current tab");
      }
      navTarget = task.conversation_url;
    } else {
      navTarget = TAB_KEY_URL;
    }
  } else {
  ...
  // Baseline only after the existing transcript has finished rendering, so a
  // late-loading prior turn isn't mistaken for the new answer (follow-up race).
  const beforeCount = await waitForTranscriptStable(page);
  const sendBtn = page
...
// After navigating to an existing conversation, ChatGPT streams the prior turns
// into the DOM over a second or two. Sample assistantCount until it stops
// growing, so the pre-send baseline reflects the FULL existing transcript — else
// a late-rendering old turn is mistaken for the new answer and a stale reply
// (e.g. the previous turn's text) gets returned on a follow-up.
async function waitForTranscriptStable(page, timeoutMs = 15000) {
  let last = -1;
  let stableHits = 0;
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const c = await page
      .evaluate(() => (window.__nyx ? window.__nyx.assistantCount() : 0))
      .catch(() => -1);
    if (c >= 0 && c === last) {
      if (++stableHits >= 2) return c;
    } else {
      stableHits = 0;
      last = c;
    }
...
async function connectWithRetry() {
  // Under launchd KeepAlive a reboot may bring this worker up before Chrome's
  // debug port is listening. Retry with backoff instead of crash-looping.
  let delay = 2000;
  for (;;) {
    try {
      return await chromium.connectOverCDP(CDP_URL);
    } catch (e) {
      log(`Chrome CDP not reachable at ${CDP_URL} (${e.message}); retrying in ${Math.round(delay / 1000)}s …`);
      await sleep(delay);
...
function isBrowserGoneError(msg) {
  return /has been closed|Target (page|closed)|browserContext|Connection closed|websocket|ECONNREFUSED/i.test(msg || "");
}
...
async function main() {
  ...
  async function establish() {
    ...
    page = await getChatPage(context);
  ...
  for (;;) {
    try {
      // Recover if Chrome was restarted underneath us (e.g. launchd relaunch
      // of the dedicated Chrome). The original connection object is dead; make
      // a fresh one rather than looping forever on "browser has been closed".
      if (!browser || !browser.isConnected()) {
        log("Chrome connection lost; reconnecting …");
        await establish();
      }
      ...
      if (resp.status === "task" && resp.task_id) {
        const runTask = (pg) =>
          resp.kind === "scrape"
            ? handleScrape(pg, resp)
            : resp.kind === "extract"
              ? handleExtract(pg, resp)
              : handlePrompt(pg, resp);
        try {
          await runTask(page);
        } catch (err) {
          // Tab/renderer crashed mid-task but the browser is still alive (common
          // on long Pro generations — an "Aw, Snap"). Re-acquire our slot tab and
          // retry the task ONCE before giving up, so a transient crash doesn't
          // fail an otherwise-good task. (Scrape excluded — let the queue retry.)
          if (resp.kind !== "scrape" && isBrowserGoneError(err.message) && browser && browser.isConnected()) {
            log(`task ${resp.task_id} hit a closed page (${err.message}); re-acquiring tab, retry 1/1`);
            try {
              page = await getChatPage(context);
              await runTask(page);
              await sleep(POLL_MS);
      ...
      // Browser/page died mid-flight (Chrome restarted) → drop the dead handle
      // so the top of the loop re-establishes a fresh connection.
      if ((browser && !browser.isConnected()) || isBrowserGoneError(err.message)) {
        log(`Chrome connection lost (${err.message}); will reconnect`);
        browser = null;
        await sleep(POLL_MS);
        continue;
      }
